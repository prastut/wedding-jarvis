  ...
  const interactive: Record<string, unknown> = {
    ...
    action: {
      buttons: buttons.map((btn) => ({
        ...
        reply: {
          id: btn.id,
          title: btn.title.substring(0, 20), // Enforce max length
        },
      })),
    },
  };
 ...
 * Button text max 20 chars, section titles max 24 chars, row titles max 24 chars
 */
export async function sendListMessage(
  ...
  const interactive: Record<string, unknown> = {
    ...
    body: { text: bodyText },
    action: {
      button: buttonText.substring(0, 20), // Enforce max length
      sections: sections.map((section) => ({
        title: section.title?.substring(0, 24),
        rows: section.rows.map((row) => ({
          id: row.id,
          title: row.title.substring(0, 24), // Enforce max length
